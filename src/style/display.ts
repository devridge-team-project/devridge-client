...
export const center = {
  ...
  screen: {
    ...
    col: (gap = 0) =>
      `gap-${gap} flex flex-col justify-center items-center min-h-screen w-full `,
    row: (gap = 0) =>
      `gap-${gap} flex flex-row justify-center items-center min-h-screen w-full `,
  },
};
...
export const box = {
  row: (height: number, gap?: number) =>
    `h-${height} gap-${gap} flex flex-row `,
