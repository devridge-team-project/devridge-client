...
export default function useValidation(nickname: string, introduction: string) {
   ...
   * - occupation: number
   */
  const [isNicknameLengthCorrect, setIsNicknameLengthCorrect] =
    useState<boolean>(false);
  const [isNicknameWordCorrect, setIsNicknameWordCorrect] =
    useState<boolean>(false);
  const [isIntroductionLengthCorrect, setIsIntroductionLengthCorrect] =
    useState<boolean>(false);
  const [isIntroductionWordCorrect, setIsIntroductionWordCorrect] =
    useState<boolean>(false);

  useEffect(() => {
    if (nickname.length < 3 || nickname.length > 12)
      return setIsNicknameLengthCorrect(false);
    if (!/^[a-zA-Z0-9가-힣]*$/.test(nickname))
      return setIsNicknameWordCorrect(false);
    if (introduction.length < 5 || introduction.length > 25)
      return setIsIntroductionLengthCorrect(false);
    if (!/^[a-zA-Z0-9가-힣,.!?]*$/.test(introduction))
      return setIsIntroductionWordCorrect(false);
